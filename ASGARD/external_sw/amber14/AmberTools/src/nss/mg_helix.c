...
int mg_helix_desc()
{
	...
	extern int mg_helix_compute();

	AVSset_module_name( "mg_helix", MODULE_DATA );
	...
	out_port = AVScreate_output_port( "o_Angle", "real" );
	AVSset_output_flags( out_port, INVISIBLE );

	out_port = AVScreate_output_port( "o_Dist", "real" );
	AVSset_output_flags( out_port, INVISIBLE );
	...
	param = AVSadd_float_parameter( "Angle", 0.0,
	...
	param = AVSadd_float_parameter( "Dist", 0.0, FLOAT_UNBOUND,
		FLOAT_UNBOUND );
	AVSadd_parameter_prop( param, "immediate", "boolean", 1 );
	AVSconnect_widget( param, "dial" );
	...
	AVSset_compute_proc( mg_helix_compute );
	return( 1 );
}
...
/* *****************************************/
int mg_helix_compute( InMatrices, OutMatrices,
	o_noID, o_Center, o_Axis, o_Angle, o_Dist, o_Count,
	noID, Axestype, Center, Axis, Angle, Dist, Count, Name
)
...
char		*Name;
{
	...
	MAT_helix( pts, &angs[0], &dst, &cnt, mats );
	...
	*o_Axis = ( char * )malloc( strlen( Axis ) + 1 );
	strcpy( *o_Axis, Axis );
	*o_Angle = ( float * )malloc( sizeof( float ) );
	**o_Angle = angs[ 0 ];
	*o_Dist = ( float * )malloc( sizeof( float ) );
	**o_Dist = dst;
	*o_Count = cnt;
