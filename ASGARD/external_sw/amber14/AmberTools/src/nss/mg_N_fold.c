...
int mg_N_fold_desc()
{
	...
	extern int mg_N_fold_compute();

	AVSset_module_name( "mg_N_fold", MODULE_DATA );
	...
	AVSset_compute_proc( mg_N_fold_compute );
	return( 1 );
}
...
/* *****************************************/
int mg_N_fold_compute( InMatrices, OutMatrices,
	o_noID, o_Center, o_Axis, o_Count,
	noID, Axestype, Center, Axis, Count, Name
)
