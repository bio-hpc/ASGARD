...
#  include "md.h"
   ...
   if ( ipb >= 1         ) write(6,9077) esurf !,edisp
   ...
   if (ipb >= 1) write(7,9077) esurf,edisp
   ...
   9059 format (1x,'EELEC  = ',f14.4,2x,'EGB     = ',f14.4,2x, &
         'RESTRAINT  = ',f14.4)
   9060 format (1x,'EELEC  = ',f14.4,2x,'EGB     = ',f14.4,2x, &
         'RESTRAINT  = ',f14.4)
...
#  include "pb_md.h"
   ...
   write(6, '()')
   write(6, '(a)') 'Dielectric Map'
   if ( ifcap /= 0 ) then
      write(6, *) '  A single spherical dielectric boundary is used'
   else
      if ( radiopt == 0 ) then
            write(6, '(a)') '  Use cavity radii in the prmtop file'
       else if ( radiopt == 1 ) then
         write(6, '(a)') '  Use Tan, Yang, and Luo optimized cavity radii definition'
      else if ( radiopt == 2 ) then
         write(6, '(a)') '  Use Pymol radii definition for potential surface display'
      else
         write(6, '(a,i2)') 'PB Bomb in pb_init(): Unknown radiopt for cavity radii', radiopt
         call mexit(6, 1)
      end if
   end if
   if ( .not. srsas ) then
      write(6, '(a)') '  The following dynamic PB radii are used at this step:'
      do iatm = 1, natom
   ...
   if ( ipb == 1 ) then
      ...
      write(6, '(x,a)') '  Use level-set-based SES definition'
      write(6, '(x,a,i12,x,a)') '    Compute SAS/SAR every', ndosas, 'steps'
      write(6, '(x,a,f19.14)') '    Solvent probe radius', dprob
      write(6, '(x,a,i8)') '    Surface dots per atom', maxsph
   else
      write(6, '(x,a)') '  Use undocumented definition'
   endif
   write(6, '(a)')
   write(6, '(x,a)') ' Boundary conditions'
   if ( bcopt == 1 ) then
      write(6, '(x,a)') '  zero potential on boundary'
   else if ( bcopt == 2 ) then
      write(6, '(x,a)') '  solute as a single DH sphere'
   else if ( bcopt == 3 ) then
      write(6, '(x,a)') '  sum of residues as independent DH spheres'
   else if ( bcopt == 4 ) then
      write(6, '(x,a)') '  sum of atoms as independent DH spheres'
   else if ( bcopt == 5 ) then
      write(6, '(x,a)') '  sum of grid charges as independent DH spheres'
   else if ( bcopt == 0 ) then
      write(6, '(x,a)') '  electrostatic focus boundary condition'
   end if
   write(6, '(a)')
   ...
   if (solvopt == 1) then
      write(6, '(x,a)') '  Use Modified ICCG solver'
   else if (solvopt == 2) then
      write(6, '(x,a)') '  Use MG solver'
   else if (solvopt == 3) then
      write(6, '(x,a)') '  Use CG solver'
   else if (solvopt == 4) then
      write(6, '(x,a)') '  Use SOR solver'
   else if (solvopt == 5) then
      write(6, '(x,a)') '  Use Adaptive SOR solver'
   else if (solvopt == 6) then
      write(6, '(x,a)') '  Use Damped SOR solver'
   end if
   write(6, '(a)')
