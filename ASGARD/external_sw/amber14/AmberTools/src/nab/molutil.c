...
REF_MATRIX_T	newtransform( REAL_T dx, REAL_T dy, REAL_T dz, REAL_T rx, REAL_T ry, REAL_T rz )
{
	...
	if( rx != 0.0 ){
		mk_idmat( r1mat );
		r1mat[ 1 ][ 1 ] = cos( D2R * rx );
		r1mat[ 1 ][ 2 ] = -sin( D2R * rx );
		r1mat[ 2 ][ 1 ] = sin( D2R * rx );
		r1mat[ 2 ][ 2 ] = cos( D2R * rx );
		concat_mat( rmat, r1mat, tmat );
		copy_mat( tmat, rmat );
	}
	if( ry != 0.0 ){
		mk_idmat( r1mat );
		r1mat[ 0 ][ 0 ] = cos( D2R * ry );
/*
		rmat[ 0 ][ 2 ] = sin( D2R * ry );
		rmat[ 2 ][ 0 ] = -sin( D2R * ry );
*/
		r1mat[ 0 ][ 2 ] = -sin( D2R * ry );
		r1mat[ 2 ][ 0 ] = sin( D2R * ry );
		r1mat[ 2 ][ 2 ] = cos( D2R * ry );
		concat_mat( rmat, r1mat, tmat );
		copy_mat( tmat, rmat );
	}
	if( rz != 0.0 ){
		mk_idmat( r1mat );
		r1mat[ 0 ][ 0 ] = cos( D2R * rz );
		r1mat[ 0 ][ 1 ] = -sin( D2R * rz );
		r1mat[ 1 ][ 0 ] = sin( D2R * rz );
		r1mat[ 1 ][ 1 ] = cos( D2R * rz );
		concat_mat( rmat, r1mat, tmat );
		copy_mat( tmat, rmat );
	}
