...
cost lap(int dim, cost **assigncost, col *rowsol, row *colsol, 
        cost *u, cost *v);


float LinearAssignment::originalLap(){
      ...
      //lazy coding: make a local copy of the cost matrix
      for( int i = 0; i < N; i++){
          for( int j = 0; j < N; j++){
              assigncost[i][j] = cm_lazy(i,j); 
          }
      }      

      lapcost = lap(N, assigncost, wellToPart, partToWell, u, v);

      //return the cost matrix again
      for( int i = 0; i < N; i++){
          for( int j = 0; j < N; j++){
              intCostMatrix[i*N+j] = assigncost[i][j] ; 
          }
          delete assigncost[i];
      }      
      delete assigncost;

      newcost = ((float)lapcost);
...
// v          - dual variables, column reduction numbers
  ...
  //needless initialisations to silence annoying compiler warnings
  last = 0;
  j2   = 0;
  min  = 0;
              ...
              col *rowsol, row *colsol, cost *u, cost *v)
{
  ...
  for (i = 0; i < dim; i++)  
    for (j = 0; j < dim; j++)  
      ...
      {
        fprintf(stderr,"\n");
        fprintf(stderr,"negative reduced cost i %d j %d redcost %d\n", i, j, redcost);
        fprintf(stderr,"\n\ndim %5d - press key\n", dim);
        if ( !scanf("%c", &wait) ) {fprintf(stderr,"Killed\n");}
        break; 
      }
  ...
  for (i = 0; i < dim; i++)  
    ...
    {
      fprintf(stderr,"\n");
      fprintf(stderr,"non-null reduced cost i %d soli %d redcost %d\n", i, rowsol[i], redcost);
      fprintf(stderr,"\n\ndim %5d - press key\n", dim);
      if ( !scanf("%c", &wait) ) {fprintf(stderr,"Killed\n");}
      break; 
    }
  ...
  for (i = 0; i < dim; i++)  
    ...
    {
      fprintf(stderr,"\n");
      fprintf(stderr,"column matched more than once - i %d soli %d\n", i, rowsol[i]);
      fprintf(stderr,"\n\ndim %5d - press key\n", dim);
      if ( !scanf("%c", &wait) ) {fprintf(stderr,"Killed\n");}
      break; 
    }
  ...
  for (i = 0; i < dim; i++)  
    ...
    {
      fprintf(stderr,"\n");
      fprintf(stderr,"error in row solution i %d soli %d solsoli %d\n", i, rowsol[i], colsol[rowsol[i]]);
      fprintf(stderr,"\n\ndim %5d - press key\n", dim);
      if ( !scanf("%c", &wait) ) {fprintf(stderr,"Killed\n");}
      break; 
    }
  ...
  for (j = 0; j < dim; j++)  
    ...
    {
      fprintf(stderr,"\n");
      fprintf(stderr,"error in col solution j %d solj %d solsolj %d\n", j, colsol[j], rowsol[colsol[j]]);
      fprintf(stderr,"\n\ndim %5d - press key\n", dim);
      if ( !scanf("%c", &wait) ) {fprintf(stderr,"Killed\n");}
      break; 
    }
