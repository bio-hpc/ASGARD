dnl Generates test code for trsv
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
include(trsv/trsv-common.m4)dnl
dnl
dnl
dnl -----------------------------------------------------------------------
dnl Usage: DO_TEST_TRSV_COMMENT(extended)
dnl        if extended, then print info about prec loop in the code 
dnl        structure
dnl -----------------------------------------------------------------------
define(`DO_TEST_TRSV_COMMENT',`
/*
 * Purpose  
 * =======
 *
...
dnl
dnl -----------------------------------------------------------------------
dnl Usage: DO_TEST_TRSV(ax_typeltr, TP_typeltr, extended)
dnl
...
dnl ----------------------------------------------------------------------
define(`DO_TEST_TRSV', `ifelse(
        `$1&&$2', `$1&&$1',`
double do_test_$1trsv$3(int n,
                     int ntests,
                     int *seed,
                     double thresh, 
                     int debug,
                     ...
                     int *num_bad_ratio,
                     int *num_tests)
DO_TEST_TRSV_COMMENT($3)
DO_TEST_TRSV_BODY($1, $2, $3) /* end of do_test_$1trsv$3 */',`
double do_test_$1trsv_$2$3 (int n, 
                          int ntests,
                          int *seed,
                          double thresh, 
                          int debug,
                          ...
                          int *num_tests)
DO_TEST_TRSV_COMMENT($3)
DO_TEST_TRSV_BODY($1, $2, $3) /* end of do_test_$1trsv_$2$3 */')') dnl
dnl
dnl
dnl
dnl --------------------------------------------------------------------
dnl Usage: DO_TEST_TRSV_BODY(ax_typeltr, TP_typeltr, extended)
dnl
dnl        ax_typeltr  : the type and precision of alpha, and x
dnl        TP_typeltr  : the type and precision of the matrix A
dnl        extended    : `' if no extended, or `_x' if extended
...
dnl        z    ... complex and double
dnl ---------------------------------------------------------------------
define(`DO_TEST_TRSV_BODY',
`{                                        
  /* function name */
  const char fname[] = "TRSV_NAME($1, $2, $3)";
          ...
          for (uplo_val=0; uplo_val<2; uplo_val++) {
            ...
            for (trans_val=0; trans_val<3; trans_val++){
              ...
              for (diag_val=0; diag_val<2; diag_val++){
                ...
                for (row=0; row<n; row++){
                  TESTGEN_TRSV_NAME($1, $2)(norm, order_type, uplo_type, 
                  ...
                  for (incx_val = -2; incx_val <= 2; incx_val++){
                    ...
                    for(j=0 ; j<n*incx_gen; j += incx_gen){
                      COPY_VECTOR_ELEMENT(x, ix, x_gen, j, $1_type)

                      ix += incx;
                    } 

                    /* call TRSV_NAME($1, $2, $3, $4) */
                    FPU_FIX_STOP;
                    TRSV_NAME($1, $2, $3)(order_type, uplo_type, trans_type, diag_type,
                              n, alpha, T, lda,
                    ...
                    for(j=0; j<n; j++){
                      if (j == row){
                        ...
                        $2tr_copy_row(order_type, uplo_type, trans_type, n, T, lda, temp, j);

                        TEST_DOT_NAME($1, $1, $2)(n, blas_no_conj,
                                      minus_one, alpha, 
                                      VECTOR_ELEMENT(x_gen, j*incx_gen, $1_type), 
                                      VECTOR_ELEMENT(x, ix, $1_type), 
                                      VECTOR_ELEMENT(r_true, j*incx_gen, EXTRA_TYPE($1_type)), 
                                      x, incx_val, temp, 1, eps_int, un_int,
                                      &ratios[j]);
                      }
                      else{
                ifelse(`$1', `s',
...
dnl
dnl --------------------------------------------------------------------
dnl Usage: TESTGEN_TRSV_NAME(ax_typeltr, T_typeltr) 
dnl        create a testgen_trsv name 
dnl --------------------------------------------------------------------
define(`TESTGEN_TRSV_NAME', `ifelse(
        `$1&&$2', `$1&&$1', `BLAS_$1trsv_testgen',
                            `BLAS_$1trsv_$2_testgen')')dnl
dnl
dnl
dnl -------------------------------------------------------------------
dnl Usage: DO_TEST_TRSV_NAME(abx_typeltr, T_typeltr, extended)
dnl        create do_test_dot name
dnl -------------------------------------------------------------------
define(`DO_TEST_TRSV_NAME', `ifelse(
        `$1&&$2', `$1&&$1',
        `do_test_$1trsv$3',
        `do_test_$1trsv_$2$3')') dnl
dnl
dnl
dnl
dnl -------------------------------------------------------------------
dnl Usage: CALL_DO_TEST_TRSV(ax_typeltr, T_typeltr, extended)
dnl
...
dnl        z    ... complex and double
dnl -------------------------------------------------------------------
define(`CALL_DO_TEST_TRSV',           
        `  fname = "TRSV_NAME($1, $2, $3, $4)";
           printf("Testing %s...\n", fname);
           ...
           for(i=0; i<nsizes; i++){
              ...
              n = sizes[i];
              total_max_ratio = DO_TEST_TRSV_NAME($1, $2, $3)(n, ntests, &seed, thresh, debug, 
