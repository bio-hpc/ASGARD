...
dnl
dnl ----------------------------------------------------------------------
dnl Usage: DOT2_TESTGEN_BODY(abr_type, x_type, y_type, int_typeltr)
...
`{  
  ...
  DECLARE_VECTOR(head_x_vec, $4_type)
  DECLARE_VECTOR(tail_x_vec, $4_type)
  ...
  MALLOC_VECTOR(head_x_vec, $4_type, 3*n)
  tail_x_vec = head_x_vec + inc*n;
  y_vec = tail_x_vec + inc*n;
  for (i = 0; i < inc*n_fix2; i += inc) {
    COPY_VECTOR_ELEMENT2(head_x_vec, $4_type, head_x_i, $2, i)
    COPY_VECTOR_ELEMENT2(tail_x_vec, $4_type, tail_x_i, $2, i)
  ...
  /* Call generator now. */
  testgen_BLAS_$4dot2(n, n_fix2, n_mix, norm,  conj,  
      PASS_BY_REF(alpha_tmp, $4_type), alpha_flag,
      PASS_BY_REF(beta_tmp, $4_type), beta_flag,
      head_x_vec, tail_x_vec, y_vec, seed, PASS_BY_REF(r_tmp, $4_type),
      r_true_l, r_true_t);
  ...
  for (i = 0; i < inc*n; i += inc) {
    COPY_VECTOR_ELEMENT2(head_x_i, $2, head_x_vec, $4_type, i)
    COPY_VECTOR_ELEMENT2(tail_x_i, $2, tail_x_vec, $4_type, i)
    COPY_VECTOR_ELEMENT2(y_i, $3, y_vec, $4_type, i)
  }

  FREE_VECTOR(head_x_vec, $4_type) /* also y_vec */
}') dnl
dnl
dnl
dnl ----------------------------------------------------------------------
dnl Usage: MIXED_DOT2_TESTGEN_BODY(abr_type, x_type, y_type, int_typeltr)
...
dnl ----------------------------------------------------------------------
define(`MIXED_DOT2_TESTGEN_BODY',
`{  
  int i;
  PTR_CAST(alpha, $1)
  PTR_CAST(beta, $1)
  ...
  if ( alpha_flag == 1 ) { /* alpha_i is fixed */
    if ( alpha_i_r == 1.0 && alpha_i_i == 0. ) { /* alpha_i == 1.0 */
      ifelse(`$2', `$3',  dnl ... both x and y_i real
        `if (beta_flag == 1 && 
        ...
        `                 dnl ... one of x and y_i is complex
         if (beta_flag == 1 && 
             ((beta_i_r == 0. && beta_i_i == 0.) || 
...
dnl
dnl ----------------------------------------------------------------------
dnl Usage: DOT2_TESTGEN_COMMENT(abr_typeltr, x_typeltr, y_typeltr)
dnl        ... generate the leading comment for the dot2 product
...
dnl ----------------------------------------------------------------------
dnl
define(`DOT2_TESTGEN_COMMENT',`
/*
 * Purpose
 * =======
 * 
 * This routine generates the test inputs to DOT2_NAME_PREFIX($1,$2,$3){_x}.
 * 
 * Arguments
 * =========
 *  
...
dnl
dnl ----------------------------------------------------------------------
dnl Usage: DOT2_NAME_PREFIX(abr_typeltr, x_typeltr, y_typeltr)
dnl        ... generate the prefix of the dot2 product routines.
...
dnl ----------------------------------------------------------------------
dnl
define(`DOT2_NAME_PREFIX', `ifelse(
     `$2&&$3', `$1&&$1',`BLAS_$1dot2', `BLAS_$1dot2_$2_$3')') dnl
dnl
dnl
define(`DOT2_TESTGEN_HEAD', 
`void DOT2_NAME_PREFIX($1, $2, $3)_testgen( int n, int n_fix2, int n_mix, dnl
     int norm, enum blas_conj_type conj, $1_array alpha, int alpha_flag, dnl
      $1_array beta,  int beta_flag, $2_array head_x, $2_array tail_x, dnl
      $3_array y, int *seed, $1_array r, double *r_true_l, double *r_true_t)')dnl
dnl
dnl
dnl ----------------------------------------------------------------------
dnl Usage: DOT2_TESTGEN(abr_typeltr, x_typeltr, y_typeltr)
dnl        ... generate the top level testing routines for dot2 product.
dnl Each type letter can be one of
dnl     s   ... real and single
dnl     d   ... real and double
dnl     c  ... complex and single
...
dnl
define(`DOT2_TESTGEN', 
 `DOT2_TESTGEN_HEAD($1, $2, $3)
  DOT2_TESTGEN_COMMENT($1, $2, $3)
  ifelse(IS_MIXED($1, $2, $3), `t', 
    `MIXED_DOT2_TESTGEN_BODY($1_type, $2_type, $3_type, REAL_ABBREV($1))', 
    `DOT2_TESTGEN_BODY($1_type, $2_type, $3_type, 
      ABBREV(REAL_COMPLEX($1_type)_`'LOWER_PREC(PREC($1_type), PREC($2_type), PREC($3_type))))')dnl
  /* end DOT2_NAME_PREFIX($1,$2,$3)_testgen */
  ') dnl
dnl
dnl
define(`PROTOTYPES', `FOREACH(`PREC_ARGS', `
DOT2_TESTGEN_HEAD(arg);')')dnl
dnl
dnl
define(`SOURCE', `dnl
#include <stdio.h>
