dnl Generates test code for hemm
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
include(hemm/hemm-common.m4)dnl
dnl
dnl
dnl  DO_TEST_HEMM($1, $2, $3, $4)
dnl    Perform the test for hemm routines.
dnl    $1 -- type of alpha, beta, and matrix C.
dnl    $2 -- type of matrix A.
dnl    $3 -- type of matrix B.
dnl    $4 -- `_x' if extended precision.
dnl    $1, $2, $3 must be one of s, d, c, z.
dnl
define(`DO_TEST_HEMM_NAME', 
`ifelse(`$1&&$1', `$2&&$3', `do_test_$1hemm$4', `do_test_$1hemm_$2_$3$4')')dnl
dnl
dnl
define(`DO_TEST_HEMM_PARAMS', 
  `int m, int n,  
   int ntests, int *seed, double thresh, int debug, float test_prob,
   double *min_ratio, double *max_ratio, int *num_bad_ratio, int *num_tests')dnl
dnl
dnl
define(`DO_TEST_HEMM_HEAD', 
  `void DO_TEST_HEMM_NAME($1, $2, $3, $4) (
        DO_TEST_HEMM_PARAMS($1, $2, $3, $4))')dnl
dnl
dnl
define(`DO_TEST_HEMM', 
  `DO_TEST_HEMM_HEAD($1, $2, $3, $4) {
   DO_TEST_HEMM_BODY($1, $2, $3, $4)
  }')dnl
dnl
dnl
dnl  TESTGEN_HEMM_NAME($1, $2, $3)
dnl    Name of the Test case generator for hemm (called by do_test_hemm)
dnl    $1 -- type of alpha, beta, matrix C.
dnl    $2 -- type of matrix A.
dnl    $3 -- type of matrix B.
dnl    each type must be one of s, d, c, z.
dnl
define(`TESTGEN_HEMM_NAME', `ifelse(
       `$2&&$3', `$1&&$1', `BLAS_$1hemm_testgen',
       `BLAS_$1hemm_$2_$3_testgen')')dnl
dnl
dnl
define(`DO_TEST_HEMM_BODY', 
  `
  /* Function name */
  const char fname[] = "HEMM_NAME($1, $2, $3, $4)";
                    ...
                    for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
                      ...
                      for (ldc_val = LDC_START; ldc_val <= LDC_END; ldc_val++) {
                        ...
                        for (randomize_val = RANDOMIZE_START; 
                            ...
                            TESTGEN_HEMM_NAME($1, $2, $3) (norm, order_type, 
                            ...
                            FPU_FIX_STOP;
                            HEMM_NAME($1, $2, $3, $4)(order_type, side_type, 
                            ...
                            for (i = 0, ci = 0, ri = 0; 
                              ...
                              $2he_copy_row(order_type, uplo_type, side_type, 
                                              m_i, a, lda, a_vec, i);
                            ...
                            if ( !(ratio <= thresh) ) {
                              ...
                              if (debug == 3) {
                                ...
                                printf("a\n");
                                $2he_print_matrix(a, m_i, lda, order_type, uplo_type);
                                ...
                                zge_print_matrix(HEAD(r_true), m, n, ldc, order_type, "HEAD(r_true)");
...
dnl
dnl
define(`CALL_DO_TEST_HEMM', 
 `fname = "HEMM_NAME($1, $2, $3, $4)";
  printf("Testing %s...\n", fname);
  total_tests = 0;
  total_bad_ratios = 0;
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    DO_TEST_HEMM_NAME($1, $2, $3, $4)(m, n, ntests, &seed, thresh, debug, 
...
dnl
FOREACH(`HEMM_ARGS', `
DO_TEST_HEMM(arg)')dnl

MAIN(`
  int m, i;
  int mn_data[NUM_DATA][2] = {{4, 4}, {2, 3}, {4, 2}, {8, 8}, 
                              {10, 1}, {1, 1}, {1, 7}};', `

FOREACH(`HEMM_ARGS', `
CALL_DO_TEST_HEMM(arg)')')dnl
