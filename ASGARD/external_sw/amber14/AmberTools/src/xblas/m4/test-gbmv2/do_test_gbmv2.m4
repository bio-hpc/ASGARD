dnl Generates test code for gbmv2
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
include(gbmv2/gbmv2-common.m4)dnl
dnl
dnl
dnl -----------------------------------------------------------------------
dnl Usage: DO_TEST_GBMV2_COMMENT(extended)
dnl        if extended, then print info about prec loop in the code 
dnl        structure
dnl -----------------------------------------------------------------------
define(`DO_TEST_GBMV2_COMMENT',`
/*
 * Purpose  
 * =======
 *
...
dnl
dnl -----------------------------------------------------------------------
dnl Usage: DO_TEST_GBMV2(abr_typeltr, x_typeltr, y_typeltr, extended)
dnl
...
dnl ----------------------------------------------------------------------
define(`DO_TEST_GBMV2', 
  `double DO_TEST_GBMV2_NAME($1, $2, $3, $4)(int m, int n, int ntests, dnl
       int *seed, double thresh, int debug, float test_prob, dnl
       double *min_ratio, int *num_bad_ratio, int *num_tests)
DO_TEST_GBMV2_COMMENT($4)
DO_TEST_GBMV2_BODY($1, $2, $3, $4)')dnl
dnl
dnl
dnl --------------------------------------------------------------------
dnl Usage: DO_TEST_GBMV2_BODY(aby_typeltr, AB_typeltr, x_typeltr, extended)
dnl
...
dnl     z  ... complex and double
dnl ---------------------------------------------------------------------
define(`DO_TEST_GBMV2_BODY',
`{                                        
  /* function name */
  const char fname[] = "GBMV2_NAME($1, $2, $3, $4)";
  ...
  DECLARE_VECTOR(x_head, $3_type)
  DECLARE_VECTOR(x_tail, $3_type)
  ...
  DECLARE_VECTOR(x_head_gen, $3_type)
  DECLARE_VECTOR(x_tail_gen, $3_type)
  DECLARE_VECTOR(y_gen, $1_type) 

  /* the true r calculated by testgen(), in double-double */
  DECLARE_VECTOR(r_true, EXTRA_TYPE($1_type))
  int alpha_val;
  int alpha_flag = 0;   /* input flag for TESTGEN_GBMV2_NAME($1, $2, $3, $4) */
  int beta_val;
  int beta_flag = 0;    /* input flag for TESTGEN_GBMV2_NAME($1, $2, $3, $4) */
  int order_val;
  ...
  *num_tests = 0;
  ...
  MALLOC_VECTOR(x_head, $3_type, max_mn*2)
  MALLOC_VECTOR(x_tail, $3_type, max_mn*2)
  MALLOC_VECTOR(y, $1_type, max_mn*2)
  MALLOC_VECTOR(x_head_gen, $3_type, max_mn)
  MALLOC_VECTOR(x_tail_gen, $3_type, max_mn)
                    ...
                    for (kl=0; kl<m; kl++) {
                      ...
                      for (lda_val=0; lda_val<3; lda_val++){
                        ...
                        if ((order_type == blas_rowmajor && lda < n) ||
                          ...
                          if (m > 0 && n > 0) 
                            TESTGEN_GBMV2_NAME($1, $2, $3)(norm, order_type, dnl
                                trans_type, m, n, kl, ku, &alpha, alpha_flag, dnl
                                AB, lda, x_head_gen, x_tail_gen, &beta, beta_flag, dnl
                                y_gen, seed, HEAD(r_true), TAIL(r_true));
                            count++;
                            ...
                            $3copy_vector(x_head_gen, n_i, 1, x_head, incx_val);
                            $3copy_vector(x_tail_gen, n_i, 1, x_tail, incx_val);
                            ...
                            for (incy_val = -2; incy_val <= 2; incy_val++){
                              ...
                              /* call GBMV2_NAME($1, $2, $3, $4) */
                              FPU_FIX_STOP;
                              GBMV2_NAME($1, $2, $3, $4)(order_type, dnl
                                  trans_type, m, n, kl, ku, alpha, AB, dnl
                                  lda, x_head, x_tail, incx_val, beta, y, incy_val dnl
                                  ifelse(`$4', `_x', `, prec'));
                              FPU_FIX_START;
                              ...
                              if ( !(ratio <= thresh) ) {
                                ...
                                if ((debug==3) &&          /* print only when debug is on */
                                  ...
                                  for (j=0, k=0; j<n_i || k<m_i; j++, k++) {
                                    if (j<n_i) {
                                      printf("      ");
                                      PRINT_ARRAY_ELEM(x_head, ix, $3_type)
                                      printf("\n");
                                    }
                                    if (j<n_i) {
                                      printf("      ");
                                      PRINT_ARRAY_ELEM(x_tail, ix, $3_type)
                                      printf("\n");
                                    }
                                    if (k<m_i) {
                                      printf("      ");
                                      PRINT_ARRAY_ELEM(y_gen, k*incy_gen, $1_type)
                                      printf("\n");
                                      printf("      ");
                                      PRINT_ARRAY_ELEM(y, iy, $1_type, y_final)
                                      printf("\n");            
                                    }
                                    ix += incx; iy += incy;
                                  }
...
ifelse(`$4', `_x', `         } /* prec */')
  ...
  FREE_VECTOR(x_head, $3_type)
  FREE_VECTOR(x_tail, $3_type)
  FREE_VECTOR(y, $1_type)
  FREE_VECTOR(x_head_gen, $3_type)
  FREE_VECTOR(x_tail_gen, $3_type)
...
dnl
dnl --------------------------------------------------------------------
dnl Usage: TESTGEN_GBMV2_NAME(aby_typeltr, AB_typeltr, x_typeltr) 
dnl        create a testgen_gbmv2 name 
dnl --------------------------------------------------------------------
define(`TESTGEN_GBMV2_NAME', `ifelse(
        `$2&&$3', `$1&&$1', `BLAS_$1gbmv2_testgen',
                            `BLAS_$1gbmv2_$2_$3_testgen')')dnl
dnl
dnl
dnl -------------------------------------------------------------------
dnl Usage: DO_TEST_GBMV2_NAME(abr_typeltr, x_typeltr, y_typeltr, extended)
dnl        create do_test_gbmv2 name
dnl -------------------------------------------------------------------
define(`DO_TEST_GBMV2_NAME', `ifelse(
        `$2&&$3', `$1&&$1',
        `do_test_$1gbmv2$4',
        `do_test_$1gbmv2_$2_$3$4')') dnl
dnl
dnl
dnl
dnl -------------------------------------------------------------------
dnl Usage: CALL_DO_TEST_GBMV2(abr_typeltr, x_typeltr, y_typeltr, extended)
dnl
...
dnl     z  ... complex and double
dnl -------------------------------------------------------------------
define(`CALL_DO_TEST_GBMV2',           
 `min_ratio = 1e308; max_ratio = 0.0;
  total_bad_ratios = 0; total_tests = 0;
  fname = "GBMV2_NAME($1, $2, $3, $4)";
  printf("Testing %s...\n", fname);
  for(i=0; i<nsizes; i++){
    m = mn_pairs[i][0];
    n = mn_pairs[i][1];
    total_max_ratio = DO_TEST_GBMV2_NAME($1, $2, $3, $4)(m, n, 1, dnl
