...
dnl
dnl
define(`GBMV2_TESTGEN_NAME', 
  `BLAS_$1gbmv2`'ifelse(`$2&&$3', `$1&&$1', `', `_$2_$3')_testgen')dnl
dnl
dnl
define(`GBMV2_TESTGEN_HEAD', 
  `void GBMV2_TESTGEN_NAME($1, $2, $3)(int norm, enum blas_order_type order, dnl
       enum blas_trans_type trans, int m, int n, int kl, int ku, dnl
       $1_array alpha, int alpha_flag, $2_array AB, int lda, $3_array x_head, dnl
       $3_array x_tail, $1_array beta, int beta_flag, $1_array y, int *seed, dnl
       double *r_true_l, double *r_true_t)')dnl
dnl
dnl
dnl ---------------------------------------------------------------------
dnl Usage: GBMV2_TESTGEN(aby_typeltr, AB_typeltr, x_typeltr)
dnl        produce gbmv_prepare signature
dnl ---------------------------------------------------------------------
define(`GBMV2_TESTGEN', 
  `GBMV2_TESTGEN_HEAD($1, $2, $3)
   GBMV2_TESTGEN_COMMENT($1, $2, $3)
   GBMV2_TESTGEN_BODY($1, $2, $3)')dnl
dnl
dnl
dnl ---------------------------------------------------------------------
dnl Usage: GBMV2_TESTGEN_BODY(aby_typeltr, AB_typeltr, x_typeltr)
dnl        produce gbmv_prepare signature
dnl ---------------------------------------------------------------------
define(`GBMV2_TESTGEN_BODY',
`{
  ...
  for(i=0; i<m_i; i++){
    /* copy AB to a_vec */
    $2gbmv_prepare(order, trans, m, n, kl, ku, AB, lda, a_vec, i, 
                   &n_fix2, &n_mix, &ysize);
    ...
    DOT2_TESTGEN_NAME($1, $3, $2)(ysize, n_fix2, n_mix, norm, dnl
        blas_no_conj, alpha, alpha_flag, beta, beta_flag, dnl
        x_head, x_tail, a_vec, seed, PASS_BY_REF(y_elem, $1_type), 
    ...
    /* copy a_vec to AB */
    $2gbmv_commit(order, trans, m, n, kl, ku, AB, lda, a_vec, i);
  }

  /* Zero out trailing part of x */
  for (i = ysize; i < n_i; i++) {
    SET_ZERO_VECTOR_ELEMENT(x_head_i, i*incx, $3_type)
    SET_ZERO_VECTOR_ELEMENT(x_tail_i, i*incx, $3_type)
  }
