dnl Generates test code for symv2
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
include(symv2/symv2-common.m4)dnl
dnl
dnl
define(`DO_TEST_SYMV2_NAME', 
  `ifelse(`$1&&$1', `$2&&$3', `do_test_$1symv2$4', `do_test_$1symv2_$2_$3$4')')
dnl
dnl
define(`DO_TEST_SYMV2_PARAMS', 
  `int n,  
...
dnl
dnl
define(`DO_TEST_SYMV2_HEAD', 
  `void DO_TEST_SYMV2_NAME($1, $2, $3, $4) 
           (DO_TEST_SYMV2_PARAMS($1, $2, $3, $4))')
dnl
dnl
define(`DO_TEST_SYMV2', 
  `DO_TEST_SYMV2_HEAD($1, $2, $3, $4) {
   DO_TEST_SYMV2_BODY($1, $2, $3, $4)
  }')
dnl
dnl
dnl
define(`TESTGEN_SYMV2_NAME', `ifelse(
       `$2&&$3', `$1&&$1', `BLAS_$1symv2_testgen', `BLAS_$1symv2_$2_$3_testgen')')
dnl
dnl
define(`DO_TEST_SYMV2_BODY', 
  `
  /* Function name */
  const char fname[] = "DO_TEST_SYMV2_NAME($1, $2, $3, $4)";
  int i;
  int yi;
  int incyi, yi0;
  int test_count;
            ...
            for (uplo_val = UPLO_START; uplo_val <= UPLO_END; uplo_val++) {
              ...
              uplo_type = (uplo_val == 0) ? blas_upper : blas_lower;
                ...
                for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
                  ...
                  TESTGEN_SYMV2_NAME($1, $2, $3) (norm, order_type, 
                    uplo_type, n, &alpha, alpha_flag, a, lda, head_x_gen, 
                    tail_x_gen, &beta, beta_flag, y_gen, seed, HEAD(r_true),
                    TAIL(r_true));
                  test_count++;
                  ...
                  for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
                    ...
                    for (incy_val = INCY_START; incy_val <= INCY_END; incy_val++) {
                      ...
                      for (i = 0, yi = yi0, ri = 0; 
                        ...
                        $2sy_copy_row(order_type, uplo_type, n, a, lda, a_vec, i);   
                      ...
                      if (  !(ratio <= thresh) ) {
                        ...
                        if (debug == 3) {
                          ...
                          printf("a\n");
                          $2sy_print_matrix(a, n, lda, order_type, uplo_type);
...
dnl
dnl
define(`CALL_DO_TEST_SYMV2', 
 `fname = "SYMV2_NAME($1, $2, $3, $4)";
  printf("Testing %s...\n", fname);
  total_tests = 0;
  total_bad_ratios = 0;
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    DO_TEST_SYMV2_NAME($1, $2, $3, $4)(n, ntests, &seed, thresh, debug,
             test_prob,   
