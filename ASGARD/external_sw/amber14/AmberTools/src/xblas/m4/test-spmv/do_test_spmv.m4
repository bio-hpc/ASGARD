dnl Generates test code for spmv
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
include(spmv/spmv-common.m4)dnl
dnl
dnl
define(`DO_TEST_SPMV_NAME', 
  `ifelse(`$1&&$1', `$2&&$3', `do_test_$1spmv$4', `do_test_$1spmv_$2_$3$4')')
dnl
dnl
define(`DO_TEST_SPMV_PARAMS', 
  `int n,  
...
dnl
dnl
define(`DO_TEST_SPMV_HEAD', 
`void DO_TEST_SPMV_NAME($1, $2, $3, $4)(
        DO_TEST_SPMV_PARAMS($1, $2, $3, $4))')
dnl
dnl
define(`DO_TEST_SPMV', 
  `DO_TEST_SPMV_HEAD($1, $2, $3, $4) {
   DO_TEST_SPMV_BODY($1, $2, $3, $4)
  }')
dnl
dnl
dnl
define(`TESTGEN_SPMV_NAME', `ifelse(
       `$2&&$3', `$1&&$1', `BLAS_$1spmv_testgen', `BLAS_$1spmv_$2_$3_testgen')')
dnl
dnl
define(`DO_TEST_SPMV_BODY', 
  `
  /* Function name */
  const char fname[] = "SPMV_NAME($1, $2, $3, $4)";
                    ...
                    for (incx_val = INCX_START; incx_val <= INCX_END; incx_val++) {
                      ...
                      for (incy_val = INCY_START; incy_val <= INCY_END; incy_val++) {
                        ...
                        for (randomize_val = RANDOMIZE_START; 
                            ...
                            TESTGEN_SPMV_NAME($1, $2, $3) (norm, order_type, 
                            ...
                            for (i = 0, yi = y_starti, ri = 0; 
                              ...
                              $2spmv_copy_row(order_type, uplo_type, 
                                              n_i, a, a_vec, i);        
...
dnl
dnl
define(`CALL_DO_TEST_SPMV', 
 `fname = "SPMV_NAME($1, $2, $3, $4)";
  printf("Testing %s...\n", fname);
  total_tests = 0;
  total_bad_ratios = 0;
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    DO_TEST_SPMV_NAME($1, $2, $3, $4)(n, ntests, &seed, thresh, debug, 
