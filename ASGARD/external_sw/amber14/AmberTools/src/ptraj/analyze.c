 ...
 *  CVS tracking:
 *
 *  $Header: /home/case/cvsroot/amber11/AmberTools/src/ptraj/analyze.c,v 10.4 2010/03/09 15:34:06 case Exp $
 *
 *  Revision: $Revision: 10.4 $
 *  Date: $Date: 2010/03/09 15:34:06 $
...
analyzeTest(analyzeInformation *analyze, stackType *sp, int mode)
{
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    argumentStackPointer = (stackType **) analyze->carg1;
    analyze->carg1 = NULL;
...
analyzeModes(analyzeInformation *analyze, stackType *sp, int mode)
{
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    state = ptrajCopyState(ptrajCurrentState());
    npair = 0;
...
analyzeTimecorr(analyzeInformation *analyze, stackType *sp, int mode)
{
  ...
  transformVectorInfo *vinfo2 = NULL;
