...
subroutine membrane_density_calc(i,j,k,xm,ym,zm,gox,goy,goz,h,&
    ...
    If (mprobe > mthick) then
        write(6,'(2a)') 'PB Warning: membrane thickness is less than',&
                    ' membrane probe radius'
    end if

    If (mthick > zm*h) then
        write(6,'(2a)') 'PB Bomb in membrane_density_calc(): membrane is thicker than',&
                   'computation grid'
        call mexit(6,1)
    end if
    ...
    !the surface of this region. If this distance is 
       ...
       dval = u
        ...
        write(6,'(a)') 'PB Bomb in membrane_density_calc(): unknown poretype'
        call mexit(6,1)
    end if
...
subroutine gen_cylinder_data(xm,ym,zm,gox,goy,goz,h,mthick,rad,&
    ...
    acount = 0
    write(6,'(a)') "building cylindrical pore exclusion region data"
    write(6,'(a,f12.4)') "radius = ",rad
    ...
    write(6,'(a,2f12.4)') "center = ",centeroid; flush(6)
    !build cylinder data
    !!write(6,*) '-debug: writting poredata';flush(6)
    do zi = 0,zm+1
...
subroutine set_membrane_eps(epsx,epsy,epsz,epsin,epsout,epsmemb,&
    ...
    if (mizmax > zm .or. mizmin < 0) then
      ...
      write(6,'(a)') 'PB Bomb in set_membrane_eps(): Membrane boundaries exceed grid boundaries'
      call mexit(6,1)
    end if
    ...
    do xi = 0,xm; do yi = 0,ym
        ...
        if (p0*px == 0) then
           if (abs(p0)+abs(px) == 0) then !membrane regular point
            ...
            ! we cowardly refuse to go any further
                  write(6,'(a)') 'PB Bomb in set_membrane_eps(): insas array memory is corrputed'
                  call mexit(6,1)
            end if
        end if
        end if
        ...
        if ( ((p0*px*py*pz) ==0) .and. ((abs(p0)+abs(px)+abs(py)+abs(pz)) /= 0 ) ) then
            ...
            if (epsx(xi,yi,zi) > epsout .or. &
                ...
                write(6,'(a)') 'PB Bomb in set_membrane_eps: epsx array value out of bounds'
                write(6,'(a,f12.4,a,3i6)') 'value: ',sngl(epsx(xi,yi,zi)),' anomalous at ',xi,yi,zi
                call mexit(6,1)
            end if
            end if
            ...
            if (epsy(xi,yi,zi) > epsout .or. &
                ...
                write(6,'(a)') 'PB Bomb in set_membrane_eps: epsy array value out of bounds'
                write(6,'(a,f12.4,a,3i6)') 'value: ',sngl(epsy(xi,yi,zi)),' anomalous at ',xi,yi,zi
                call mexit(6,1)
            end if
            end if
            ...
            if (epsz(xi,yi,zi) > epsout .or. &
                ...
                write(6,'(a)') 'PB Bomb in set_membrane_eps: epsz array value out of bounds'
                write(6,'(a,f12.4,a,3i6)') 'value: ',sngl(epsz(xi,yi,zi)),' anomalous at ',xi,yi,zi
                call mexit(6,1)
            end if
            end if
        end if
...
subroutine exclusion_calculation(i,j,k,gox,goy,goz,h,xm,ym,zm,&
    ...
    if (poretype == 1) then !cylinderical region
        ...
        write(6,'(a)') 'PB Warning: Automated void checking not yet implemented'
        eval = 1.0d0
    else if (poretype == 3) then    !spherical exclusion region
        write(6,'(a)') 'PB Warning: Spherical exlusion regions not yet implemented'
        eval = 1.0d0 
    else if (poretype == 4) then    !load from dx file
        write(6,'(a)') 'PB Warning: External void region files not yet implemented'
        eval = 1.0d0
    else
        write(6,'(a)') 'PB Bomb in exclusion_calculatin(): Unknown exclusion type'
        call mexit(6,1)
    end if
