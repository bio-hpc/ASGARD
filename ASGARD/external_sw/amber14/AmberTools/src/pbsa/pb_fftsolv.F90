...
#  include "pb_constants.h"
    ...
    !  10 (+512)    write_progress      -   Turns on / off write(6,'(a)')ing of progress to mdout
    ...
    !as well (this should be obvious, but is in place as a precaution).
        ...
        write(6,'(a)') '-debug: pb_fftsolv: initializing';Flush(6)
...
!    write(6,'(a)') 'Setting flags'
    ...
    if (write_progress) then
        write(6,'(a,l)') 'FD Green function checking Flag set to: ',fdcheck_flag
        write(6,'(a,l)') 'Phi grid writting flag set to: ',genfile_phigrid_flag
        write(6,'(a,l)') 'Charge grid writting Flag set to: ',genfile_cgrid_flag
        write(6,'(a,l)') 'Use grid based fd Flag set to: ',grid_based_fd
        write(6,'(a,l)') 'Charged node arrays writting Flag set to: ',genfile_grdcrg_flag
        write(6,'(a,l)') 'Charged node potential writting Flag set to: ',genfile_grdphi_flag
        write(6,'(a,l)') 'Charged node array overwritting disablement Flag set to: ',internal_grdcrg
        write(6,'(a,l)') 'Surface charge density generation Flag set to: ',gen_surface_charge
        write(6,'(a,l)') 'Surface charge density writting Flag set to: ',genfile_surface_charge
        flush(6)
    end if
   ...
   if (write_progress) then
    write(6,'(a)') 'entering array allocation block'
    flush(6)
   end if
   if (internal_grdcrg) then
    ...
    if (write_progress) then
        write(6,'(a,i6,a)') 'there are ',tempn,' charged gridnodes'
        write(6,'(a)') 'allocating memory for local charged grid node data arrays'
        flush(6)
    end if
      allocate ( grdcrg_l(3,tempn), stat=ist)
      if ( ist /= 0 ) then 
         write(6,'(a,2i6)') 'allocate grdcrg_l(3,tempn) failed',tempn,ist
         stop
        else
        if (write_progress) then
         write(6,'(a)') 'allocation of grdcrg_l succeeded'
         flush(6)
        endif
      end if
     allocate( cgmag(tempn), stat=ist )
      if ( ist /= 0 ) then 
         write(6,'(a,2i6)') 'allocate cgmag(tempn) failed',tempn,ist
         stop
        else
        if (write_progress) then
         write(6,'(a)') 'allocation of cgmag succeeded'
         flush(6)
        endif
      end if
     if (write_progress) then
        write(6,'(a)') 'building charged grid node arrays from charge grid'
        flush(6)
     end if
    call build_charge_arrays(cggrid,grdcrg_l,cgmag,ngrdcrg_l,nx,ny,nz)
     ...
     tempn = SIZE(grdcrg,1)
     if (write_progress) then
        write(6,'(a,i6,a)') 'grdcrg holds ',tempn,' node index sets'
        write(6,'(a)') 'allocating memory for local charge grid node data arrays'
        flush(6)
     end if
      allocate ( grdcrg_l(3,tempn), stat=ist)
      if ( ist /= 0 ) then 
     ...
     call build_charge_arrays(cggrid,grdcrg_l,cgmag,ngrdcrg_l,nx,ny,nz)
     if (write_progress) then
        write(6,'(a)') 'writting local arrays to global copies'
        flush(6)
     end if
     grdcrg = grdcrg_l
     ngrdcrg = ngrdcrg_l
   ...
   if (write_progress) then
    write(6,'(a)') 'initializing phi grid format'
    flush(6)
   end if
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    tempcrg=SUM(cggrid)
    if (write_progress) then
        write(6,'(a)') 'Analyzing grid data'
        write(6,'(a,f12.4)') 'Net charge of cggrid: ', tempcrg
        write(6,'(a,f12.4)') 'fill ratio set to: ', fillratio
        write(6,'(a,3i6)') 'nodes per dim. x,y,z ',nx,ny,nz
        write(6,'(a,f12.4)') 'max net charge magnitued threshold is:',nx*ny*nz*1.0d-15 
        flush(6)
    end if
   if (abs(tempcrg) >= (nx*ny*nz*1.0d-15)) then
        if (write_progress) then
                write(6,'(a)') 'net charge is greater than threshold'
                write(6,'(a)') 'neutralizing grid'
                flush(6)
        end if
       ...
       ! end do; end do; end do;
        if (write_progress) then
            write(6,'(a,f12.4,a)') 'added ',tempcrg/(nx*ny*nz),'to each grid node'
            write(6,'(a,f12.4)') 'net charge is now: ',SUM(cggridtemp)
        end if 
   else
        cggridtemp = cggrid
   end if
   ...
   gridhz = gridh
    if (write_progress) then
        write(6,'(a,f12.4)') 'grid cell volume= gridhx*gridhy*gridhz = ',sngl(gridhx*gridhy*gridhz)
        write(6,'(a,3f12.4)') 'gridspacing (x,y,z)', sngl(gridhx),sngl(gridhy),sngl(gridhz)
        flush(6)
    end if
   if ( genfile_cgrid_flag ) then !write charge grid to file if debugging
    if (write_progress) then 
        write(6,'(a)') 'Writting Charge Grid Data File'; flush(6) 
    end if
    call genfile_cgrid(cggridtemp,nx,ny,nz) 
    write(6,'(a)') 'done writting'; flush(6)
   end if
   if ( genfile_grdcrg_flag ) then !write charged node data arrays to file
        if (write_progress) then
            write(6,'(a)') 'Writting charged node data file'; flush(6)
            flush(6)
        end if
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if ( write_progress ) then
        write(6,'(a)') 'calling pb_fft'; flush(6)
    end if
    write(6,'(a)') '-debug: pb_fftsolv: calling pb_fft';flush(6)
...
#else
    write(6,'(a)') "PBSA FFT solver is disabled, probably because gcc <= 4.2"
#endif
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   if ( fdcheck_flag ) then !fd calculation, used for debugging
    if (write_progress) then
        write(6,'(a)') 'generating fd potential map'
        call flush(6)
    end if
   ...
   if ( genfile_phigrid_flag ) then
        if (write_progress) then
            write(6,'(a)') 'writting potential map data files'
            call flush(6)
        end if
        if ( fdcheck_flag ) then
   ...
   if ( genfile_grdphi_flag ) then
        if (write_progress) then
            write(6,'(a)') 'writting charged node potential data files'
            call flush(6)
        end if
        if ( fdcheck_flag ) then
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if (write_progress) then
        write(6,'(a)') 'deallocating arrays'
        call flush(6)
    end if 
    deallocate(grdcrg_l)
    deallocate(cgmag)
    if (write_progress) then
        write(6,'(a)') 'exiting pb_fftsolv'
        call flush(6)
    end if
contains
...
subroutine gen_fd_phi_grid(cggrid,fd_phi_grid,grdcrg_l,cgmag,ngrdcrg_l,gridh,&
    ...
    if ( (abs(tempcrg) <= nx*ny*nz*1.0d-32) .AND. grid_based_fd ) then 
        write(6,'(2a)') 'net grid charge magnitude within threshold and grid based fd on.',&
                   ' Using grd arrays'
    do xi=1,nx; do yi=1,ny; do zi=1,nz
    ...
    call flush(6)
        ...
        write(6,'(2a)') 'net grid charge maginitude too high or grid based fd off.',&
                   ' Using full grid fd'
        do xi=1,nx; do yi=1,ny; do zi=1,nz
        ...
        write(6,'(a)') 'done making fd_phi_grid'; flush(6)
...
subroutine add_atom_potential(eel,enAt,cggrid,atom,atomcoords,cgmag,phi_grid,pb_bounds,gridhx,gridhy,gridhz,nx,ny,nz)
   ...
   !if ( fdcheck_flag ) then
   write(6,'(a,i6)') 'adding potential for atom ', atom
   write(6,'(a,3i6)') '   grid node: ', xindex, yindex, zindex
   write(6,'(a,3f12.4)') '   location: ', sngl(atomcoords(1,atom)),sngl(atomcoords(2,atom)),sngl(atomcoords(3,atom))
   write(6,'(a,f12.4)') '   phi: ', phi_grid(xindex,yindex,zindex);
   write(6,'(a,f12.4)') '   node self potential: ', self_energy
   !write(6,'(a)') '   corrected phi: ', (phi_grid(xindex,yindex,zindex) - cgmag(atom) / (dconst*hvol))
   write(6,'(a,f12.4)') '   atom charge: ', cgmag(atom)
   write(6,'(a,f12.4)') '   node charge: ', cggrid(xindex,yindex,zindex)
   write(6,'(a,f12.4)') '   uncorrected energy: ', phi_grid(xindex,yindex,zindex)*cgmag(atom)&
                                         *AMBER_ELECTROSTATIC2
   write(6,'(a,f12.4)') '   corrected energy:   ', phi_grid(xindex,yindex,zindex)*cgmag(atom)&
