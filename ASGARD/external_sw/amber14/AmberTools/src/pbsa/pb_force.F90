   ...
   if ( multiblock ) then
      ...
      ! Apparently this is for expert use only
         call pb_fdfrc(pbverbose,pbprint,pbgrid,ifcap,ipb,imin,natom,natom,npdec,idecomp,irespw, &
                 ipres,jgroup,ibgwat,ibgion,pbfrc,eelrffd,ionene,npbstep,npbgrid,nstlim) 
   ! else
   !    write(6,'(a)') "Exception caught in pb_force."; call mexit(6,1)
   end if
   call pbtimer_stop(PBTIME_PBFDFRC)
...
#  include "parallel.h"
   ...
   do m = 1, mpdec
      ...
      !xs shouldn't be reset in MD/MC, to be fixed
...
#else
            write(6,'(a)') "PB_AUGFFT is not supported in SANDER";call mexit(6,1)
#endif
         else
            write(6,'(a)') "PB Bomb: pb_fdfrc: unknown ipb";call mexit(6,1)
         end if

! MJ: Ligand debugging statements
!        if ( level == nfocus ) then
!          if ( ligand ) then 
...
! XP: augdrv debugging statements
...
!   end do
         ...
         if ( outphi .and. level == nfocus ) then
            ...
            ! write dx format phi
               ...
               write(6,'(a)') 'Warning, unrecognizable phimap output format.'
      ...
      if ( eneopt == 1 ) then
         ...
         if ( intopt == 2 ) then
            if ( bcopt < 6 .or. bcopt > 9 ) then
               write(6,'(a)') 'PB Bomb in pb_force(): intop = 2 does not support bcopt < 6'
               call mexit(6, 1)
            end if
         ...
         if ( frcopt == 1 ) then
            ...
            ! RL: DBF printing is now done inside the routine
            !if ( pbverbose .and. pbprint ) then
            !   write(102,'(a)') ' :::: Atomic DB forces ::::'
            !   do iatm = 1, natom
            !      write(102,'(3e20.6)') pbfrc(1:3,iatm)*frcfac
            !   end do
            !   close(102)
            ...
            ! RL: for zeroing out net force when force interpolation is not
            ! precise.
            !fnet = ZERO
            !do iatm = 1, atmlast
      ...
      ! probably a uniform dielectric run for reference state calculation or development
...
!write(3000,'(2x,a,a6,f9.5,2f15.5)') "H",str,crd(1:3)
!if ( ip == 2982 ) then
!  write(6,'(a)')"mjhsieh: ",insas(i,j,k),i,j,k,g
!  write(6,'(a)')"mjhsieh: ",iepsav(4,ip),arccrd(1:3,iepsav(4,ip))
! write(6,'(a)')"mjhsieh: ",dot_product(crd(1:3)-arccrd(1:3,iepsav(4,ip)),crd(1:3)-arccrd(1:3,iepsav(4,ip)))
! write(6,'(a)')"mjhsieh: ",g
! iarc = dotarc(iepsav(4,ip)) ! this leads to the circle/arc
! write(6,'(a)') 'the two atoms forming the arc', iarc, arcatm(1:2,iarc)
!end if
!end if
...
subroutine pb_dbfrc_fld2( verbose,eneout,natom,eelrf,f,epsx,epsy,epsz,zv,phi,chgrd,cphi )
   ...
   do ip = 1, nbndx
      ...
      if ( iatm == 0 ) then
         x(1:3) = ZERO
         write(6,'(a)') 'PBMD FATAL ERROR: can not find projection atom/probe' 
         call mexit(6, 1)
      elseif ( iatm > 0 ) then
         x(1:3) = acrd(1:3,iatm)
         rsphere = radi(iatm)
