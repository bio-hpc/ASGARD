...
subroutine thermo(natoms,nvecs,ilevel,idecomp,c,amass,freq,vtemp,evibn, &
      cvibn,svibn,t,patm)
   use decomp
...
#ifdef PRINT_VTEMP
   itop = min0(ndof,5)
   write(iout,1110) (vtemp(i),i=1,itop)
   if (ndof <= 5) write(iout,1120)
   itop = min0(ndof,10)
   if (ndof > 5) write(iout,1120) (vtemp(i),i=6,itop)
   if (ndof > 10)write(iout,1125) (vtemp(i),i=11,ndof)
#endif
   ...
   if (lofreq == 0) goto 240
   do 230 i=1,lofreq
      write(iout,1190) i,evibn(i),cvibn(i),svibn(i)
   230 continue
#endif
   ...
   do 280 i=1,ndof
      ...
      if(idecomp == 1) then
         call decmode(2,i,svibn(i),natoms)
      end if
   280 continue
