...
subroutine qm_mm(coords,natom,scaled_mm_charges,f,escf, &
                 ...
                 ntype,atom_name,atom_type, atomic_mass, atom_type_index, nstep) 
!
...
!=============================================================================
   ...
   if (abfqmmm_param%abfqmmm == 1) qmmm_struct%qm_mm_first_call = .true.
...
!======================END OF QMMM SETUP ======================================
   ...
   if (qmmm_nml%qmtheory%EXTERN) then
      ...
      call qm2_extern_get_qm_forces(nstep, qmmm_struct%nquant_nlink, &
           qmmm_struct%qm_coords, qmmm_struct%iqm_atomic_numbers, &
...
!atoms.
   ...
   do i=1,qmmm_struct%nlink
     ...
     call distribute_lnk_f(forcemod,qmmm_struct%dxyzqm(1:3,qmmm_struct%nquant+i),coords(mm_no), &
...
subroutine get_qm2_forces(master, calc_mchg_scf, natom, &
   ...
   if (first_call) then
      ...
      if (master) then
         ...
         if(abfqmmm_param%abfqmmm /= 1 .or. (abfqmmm_param%qmstep == 1 .and. abfqmmm_param%system == 1)) &
   ...
   if ( qmmm_nml%qmmm_int>0 .and. (qmmm_nml%qmmm_int /= 5) ) then
      ...
      if (qmmm_nml%qmtheory%DFTB) then
         ! Parallel
         call qm2_dftb_get_qmmm_forces(qmmm_struct%dxyzcl,qmmm_struct%dxyzqm, qmmm_scratch%qm_real_scratch, &
              qmmm_scratch%qm_real_scratch(natom+1:natom+qmmm_struct%qm_mm_pairs), &
              qmmm_scratch%qm_real_scratch(2*natom+1:2*natom+qmmm_struct%qm_mm_pairs), &
              qmmm_scratch%qm_real_scratch(3*natom+1:3*natom+qmmm_struct%qm_mm_pairs))
      else
         ! Parallel
