...
subroutine get_nb_energy(iac,ico,ntypes,charge, &
      cn1,cn2,cn6,force,numatoms, ipairs, &
...
#ifdef HAS_10_12
      epol,dipole,field,mpoltype,asol,bsol)
#else
            ...
            if ( ntot > 0 )then
               if ( mpoltype == 0 )then
                  ...
                  call short_ene(i,xk,yk,zk,ipairs(numpack),ntot,nvdw,nhbnd, &
                                 eedtbdns,eed_cub,eed_lin,charge, &
                                 ntypes,iac,ico,cn1,cn2,cn6,filter_cut, &
                                 eelt,evdw,force,dir_vir,ee_type,eedmeth,dxdr, &
                  ...
                  call short_ene_dip(i,xk,yk,zk,ipairs(numpack),ntot,nvdw, &
                        ewaldcof,eedtbdns,eed_cub,eed_lin,charge,dipole, &
                        ntypes,iac,ico,cn1,cn2,cn6,filter_cut, &
                        eelt,epol,evdw,ehb,force,field,pol,pol2,dir_vir, &
#ifdef HAS_10_12
                        ee_type,eedmeth,mpoltype,dxdr,eedvir,cn3,cn4,cn5,asol,bsol)
#else
                        ee_type,eedmeth,mpoltype,dxdr,eedvir,cn3,cn4,cn5)
#endif
               end if
...
subroutine short_ene(i,xk,yk,zk,ipairs,ntot,nvdw,nhbnd, &
                     eedtbdns, eed_cub,eed_lin,charge, &
                     ntypes,iac,ico,cn1,cn2,cn6,filter_cut, &
                     eelt,evdw,force,dir_vir, ee_type,eedmeth,dxdr,eedvir, &
                     cache_df,cache_x,cache_y,cache_z,cache_r2,cache_bckptr, &
#ifdef HAS_10_12
                     cn3,cn4,cn5,asol,bsol,ehb)
#else
                     cn3,cn4,cn5)
#endif
   ...
   use nbips, only: teips,tvips,nnbips,rips2,ripsr,rips2r,rips6r,rips12r, &
                    aipse,aipsvc,aipsva,bipse,bipsvc,bipsva,pipsec,pipsvcc, &
                    pipsvac
#ifdef MPI /* SOFT CORE */
   use softcore, only : scalpha, scbeta, sigma6, foureps, sc_dvdl, isProcessV1, &
        sc_ener, nsc, oneweight, weight0, weight1, sceeorder, sc_dvdl_ee, emil_sc
...
#endif
   _REAL_ mr4, f4
   _REAL_ delx,dely,delz,delr, delr2, cgi, cgj, &
      ...
      do im_new = 1, icount
         ...
         if ( nsc(i) == nsc(j) .and. emil_sc .eq. 0) then          
            ...
            if ( isProcessV1 ) then
               ...
               sc_dvdl_ee = sc_dvdl_ee + switch / sceeorder * comm1 * denom_n * scbeta
...
#  ifndef noVIRIAL
         ee_vir_iso = ee_vir_iso - dfee * cache_r2(im_new) ! include the softcore contribution to the ee virial
#  endif
      end do  !  im_new = 1, icount
...
subroutine short_ene_dip(i,xk,yk,zk,ipairs,numtot,numvdw, &
      ...
      ntypes,iac,ico,cn1,cn2,cn6,filter_cut, &
      eelt,epol,evdw,ehb,frc,field,pol,pol2,dir_vir, &
#ifdef HAS_10_12
      ee_type,eedmeth,mpoltype,dxdr,eedvir,cn3,cn4,cn5,asol,bsol)
#else
      ee_type,eedmeth,mpoltype,dxdr,eedvir,cn3,cn4,cn5)
...
#include "../include/md.h"
   ...
   _REAL_ eedtbdns,filter_cut,dxdr
   integer ntypes,iac(*),ico(*)
   _REAL_ cn1(*),cn2(*),cn6(*),eelt,epol,evdw,ehb,frc(3,*),field(3,*),pol(*)
   _REAL_ pol2(*)
   _REAL_ cn3(*),cn4(*),cn5(*)
#ifdef HAS_10_12
   _REAL_ asol(*), bsol(*)
   ...
   integer, parameter :: mask27 = 2**27 - 1
   _REAL_ mr4, f4
   _REAL_ delx,dely,delz,delr, delr2, f10, r10, cgi, delr2inv, r6, f6, f12, &
          df, dx, x, dfx, dfy, dfz, dumx, dumy, dumz
   integer itran
! M-WJ
