...
do im_new = 1,icount
   ...
   if ( nsc(i) == nsc(j) .and. emil_sc .eq. 0) then
      ...
      denom = 1.0d0 / ( scalpha * clambda + r6 * b0 ) !b0 is 1/(sigma^6)
      denom2 = denom * denom
      denom3 = denom2 * denom
      ...
      ! -- ti decomp
      if(decpr .and. idecomp > 0) call decpair(3,i,j,weight0*b1*(2.0d0*scalpha*denom3 - scalpha*denom2)/(nstlim/ntpr))
      ...
      if (ifmbar /= 0 .and. do_mbar) then
         ...
         do bar_i = 1, bar_states
            denom = 1.0d0 / ( scalpha * bar_lambda(bar_i) + r6 * b0 ) !b0 is 1/(sigma^6)
            denom2 = denom * denom
            bar_cont(bar_i) = bar_cont(bar_i) + b1 * ( denom2 - denom )
         end do
      end if
