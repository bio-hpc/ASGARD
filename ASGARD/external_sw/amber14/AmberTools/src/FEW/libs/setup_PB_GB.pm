...
# deletion of all residues except the recepter residues (and the ligand in case
...
sub gen_leap_in_top_setup{
	...
	while(my $l = <LEAP_MD>){
		...
		if($l =~ m/source leaprc.ff99SB/){
			if($c_in{'backwards'} == 0){
				$l =~ s/leaprc.ff99SB/leaprc.ff12SB/;
				print NEW_LEAP $l."\n";
				print NEW_LEAP "loadAmberParams frcmod.ionsjc_tip3p\n";
			}
			else{
...
sub modify_extract_snaps{
	...
	foreach my $tag (@tags){
		...
		while(my $coord = <COORD_TEMPL>){
			...
			# ligand in case of 3-trajectory approach
			if($coord =~ m/^RECEPTOR/){
				$coord =~ s/0/1/;
			}
...
sub modify_mmpbsa_in{
	...
	while(my $mmpbsa_c = <MMPBSA_TEMPL>){
		...
		if(($c_in{'Decomp'} > 0)&&(($c_in{'GB'} != 0)||($c_in{'ImplMem'} == 1))){
			if($mmpbsa_c =~ m/^DC /){
				$mmpbsa_c =~ s/0/1/;
			}
			if($mmpbsa_c =~ m/^MS/){
