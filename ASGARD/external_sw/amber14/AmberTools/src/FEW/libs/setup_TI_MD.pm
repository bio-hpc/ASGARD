...
sub setup_leap_for_lib_TI{
	...
	if($c_in{'backwards'} == 0){
		print LEAP_LIB "source leaprc.ff12SB\n";
		print LEAP_LIB "loadAmberParams frcmod.ionsjc_tip3p\n";
	}
	else{
...
sub gen_leap_crd_top_in{
	...
	open(LEAP_CT, ">$o_dir/leap_top_crd.in") || die "Cannot open leap_top_crd.in for writing.\n";
	if($c_in{'backwards'} == 0){
		print LEAP_CT "source leaprc.ff12SB\n";
		print LEAP_CT "loadAmberParams frcmod.ionsjc_tip3p\n";
	}
	else{
	...
	for my $name ($c_in{'Lname_start'}, $c_in{'Lname_end'}){
		for my $tag ("_com", "_lig") {
			...
			if(($name.$tag eq $c_in{'Lname_end'}."_com")||($o_dir."/".$name.$tag eq $c_in{'Lname_start'}."_com")){
				print LEAP_CT "\n";
			}
		}
...
sub equi_setup_TI{
	...
	my $prod_no = 1/$requested_prod_time_templ;
	my $first_lambda = 1;
	...
	foreach my $lambda (@lambda){
		...
		for my $equi_templ_file (@equi_templ_file){
			...
			for my $pre ($c_in{'Lalias_start'}, $c_in{'Lalias_end'}){
				my $str_out = "";
				...
				if($pre eq $c_in{'Lalias_start'}){		
					...
					open(EQUI_IN, ">".$tag_dir."/".$pre."_equi0$E"."_v1_l$lambda.in") || die "Cannot open file $pre"."_equi1$E"."_v0_l$lambda.in\n";
				}
...
sub check_TIequi{
	...
	foreach my $tag (@tags){
		...
		foreach my $file (@TIequi_out_files){
			...
			if($basename[$#basename] =~ m/$end_alias/){
				
				$basename[$#basename] =~ m/\_v1\_l(\d+)/;
				my $lambda = $1;
				my %lambdas;
		...
		my ($dVdL_ref_equi, $time_inter_ref_equi, $dVdL_value_no_ref_equi) = read_dVdL_data($last_equi_file_no, "equi", $path_to_file, \@sorted_lambdas, \%TIequi_out_files);
		
		my %dVdL_equi = %{$dVdL_ref_equi};
		my %time_inter_equi = %{$time_inter_ref_equi};
		my %dVdL_value_no_equi = %{$dVdL_value_no_ref_equi};
		
		my ($dVdL_ref_prod, $time_inter_ref_prod, $dVdL_value_no_ref_prod) = read_dVdL_data($last_prod_file_no, "prod", $path_to_file, \@sorted_lambdas, \%TIequi_out_files);
	...
	foreach my $l (@sorted_lambdas){
		...
		for(my $i=1; $i<=$last_file_no; $i++){
			...
			while(my $f_line = <OUT_FILE>){
				...
				if(($begin_read == 1)&&($f_line =~ m/NSTEP =\s+(\d+)   TIME\(PS\) =\s+(\d+)\.(\d+)/)){
					$time = $2.".".$3;
...
sub detect_last_equi_basename{
	...
	foreach my $rst_file (@equi_rst_files){
		...
		if($filename_components[$#filename_components] =~ m/prod(\d+)\_v1\_l(\d+)\.rst/){
			my $curr_no = $1;
			my $lambda = $2;
			if(! $rst_no{$curr_no}){
				push(@rst_no, $curr_no);
	...
	foreach my $l (keys %lambdas){
		if(! -e $equi_path_and_name.$last_rst_file_base."_v1_l".$l.".rst"){
			...
			print $equi_path_and_name.$last_rst_file_base."_v1_l".$l.".rst\n";
			exit;
		}
	}
