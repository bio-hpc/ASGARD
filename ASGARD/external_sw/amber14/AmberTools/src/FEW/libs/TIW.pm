	...
	if($c_in{'ti_sim'} == 1){
		...
		if($c_in{'crd_top'} ==  1){
			...
			&&(! -e "$setup_dir/$c_in{'Lname_start'}"."_com_TIin.crd")&&(! -e "$setup_dir/$c_in{'Lname_end'}"."_com_TIin.crd")){
				...
				gen_leap_crd_top_in($setup_dir, \%c_in, $ref_cys_bridges);
			...
			if(($c_in{'prod_templ'})&&(-e $c_in{'prod_templ'})){
				...
				if(! -e $equi_tag_dir){
					...
					foreach my $tag (@tags){
						...
						my $equi_path_and_base = $equi_tag_dir."/".$tag."/".$c_in{'Lalias_end'}."_";
						my $last_equi_basename = detect_last_equi_basename($equi_path_and_base, \@prod_lambda);
		
						# Copy topology and coordinate files to TI MD-directory
						for my $name ($c_in{'Lalias_start'}, $c_in{'Lalias_end'}){
					
							#Copy topology
							my $q_topo = $equi_tag_dir."/".$tag."/".$name."_".$tag."_TIin.top";
							my $t_topo = $MD_tag_dir."/".$name."_".$tag."_TIin.top";
							...
							foreach my $l (@prod_lambda){
								my $q_crd = $equi_tag_dir."/".$tag."/".$name."_".$last_equi_basename."_";
						...
						# Create MD-setup					
						generate_cpptraj_img($MD_tag_dir, $tag, $com_res_lig, $mask);
			...
			foreach my $lambda (@sorted_lambdas){
				...
				foreach my $prod_no (@sorted_prods){
					...
					while(my $dVdL_line = <DVDL_SRC>){
						...
						if(($read_dVdL == 1)&&($dVdL_line =~ m/NSTEP =\s+(\d+)   TIME\(PS\) =\s+(\d+)\.(\d+)/)){
							$curr_time = $2.".".$3;
						}
						
						if(($read_dVdL == 1)&&($time_read == 0)&&($dVdL_line =~ m/NSTEP =\s+(\d+)   TIME\(PS\) =\s+(\d+)\.(\d+)/)){
							my $time = $2.".".$3;
						...
						if(($read_dVdL == 1)&&($dVdL_line =~ m/^ DV\/DL/)){
