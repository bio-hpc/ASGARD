...
sub check_input_common{
	...
	elsif(((! $c_in{'fi_lig'})||($c_in{'fi_lig'} eq ""))&&(($c_in{'i_path'} eq "")||(! -e $c_in{'i_path'}))){
...
sub check_input_wamm{
	...
	elsif(($c_in{'analyze'} == 1)&&(($c_in{'chrg_meth'} ne "am1")&&($c_in{'chrg_meth'} ne "resp"))){
	...
	elsif(($c_in{'analyze'} == 1)&&((! $c_in{'rec_res'})||($c_in{'rec_res'} eq ""))){
	...
	elsif(($c_in{'analyze'} == 1)&&($c_in{'PB'} == 1)&&(($c_in{'GB'} == 2) || ($c_in{'GB'} == 5))){
	...
	elsif(($c_in{'analyze'} == 1)&&($c_in{'PB'} == 4)&&(($c_in{'GB'} == 2) || ($c_in{'GB'} == 5))){
	...
	if($check == 1){
		...
		if(($c_in{'analyze'} == 1)&&($c_in{'ImplMem'} == 1)){
...
sub check_mol2{
	...
	if(grep {$struct_name eq $_} @ff_res_names){
		if($c_in{'backwards'} == 0){
			...
			print "ff12SB force field or ions13.lib file of AMBER. Such a ligand name cannot be used for\n";
			print "calculation setup with FEW.\n";
			print "Please change the ligand name to a name not pre-defined in the ff12SB force field\n";
			print "of AMBER.\n";
		}
		else{
			print "ERROR:\n";
