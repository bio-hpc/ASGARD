...
sub WAMM{
	...
	if($c_in{'analyze'} == 1){
		...
		if($c_in{'chrg_meth'} eq "resp"){
			$analyze_dir = $c_in{'root_path'}."/calc_r_".$c_in{'traj'}."t";
			$chrg_alias = "r";
		}
		if($c_in{'chrg_meth'} eq "am1"){
		...
		foreach my $s (@struct_files){
			...
			if((!$c_in{'coord_templ'})||($c_in{'coord_templ'} eq "")){
				$c_in{'coord_templ'} = $current_dir."/examples/input_info/extract_snaps.in";
			}
			...
			if((-e $c_in{'coord_templ'})&&($c_in{'get_snaps'} == 1)){
				...
				my ($lig_atom_start, $lig_atom_end, $rec_atom_start, $rec_atom_end, $total_atoms) = identify_atom_no($struct_b{$s}, \%c_in);
			...
			if((! $c_in{'mmpbsa_templ'})||($c_in{'mmpbsa_templ'} eq "")){
				$c_in{'mmpbsa_templ'} = $current_dir."/examples/input_info/mmpbsa.in";
			}
