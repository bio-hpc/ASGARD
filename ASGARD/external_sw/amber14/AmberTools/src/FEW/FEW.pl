...
# with tools available in AMBER14
...
BEGIN{
	...
	if($amber_home eq ""){
		...
		my $perlmol_lib_path = $amber_home."/AmberTools/src/FEW/additional_libs/PerlMol/";
...
if($current_dir =~ m/bin/){
	my @current_dir_sep = split(/\//, $current_dir);
	my $new_current_dir = "";
	for(my $i=0; $i<$#current_dir_sep; $i++){
		$new_current_dir .= $current_dir_sep[$i]."/";
	}
	$new_current_dir .= "AmberTools/src/FEW";
