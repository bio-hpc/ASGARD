...
def find_name_rec_lig(folder, cl):
    """
        Devuelve el nombre de la query
    """    
    pattern = folder + "/{}/{}_*{}"
...
"""
   Leemos los ficheros de clusters de todos las carpetas pasadas,
   se ordenar de mejor score o cluster
...
for i in array_bd_cl_folders:
    ...
    if len(data) > 0:
        ...
        #if  header == "":
        #    header = format_text_coord.format('Cluster '+name_target)
        aux_data = []
        for i in data:
            aux_data.append(i[1])    
        data = aux_data       
        ...
        if not lig_name in array_bd_cl:   
            ...
            #header += format_text.format(lig_name)
        else:
            d = array_bd_cl[lig_name]    
            d += data
            array_bd_cl[lig_name] = d

#   Ojo esta ordenacion es importante y no esta del todo probada
array_bd_cl = OrderedDict( sorted(array_bd_cl.items(),  key=key_2)    )
for i, lst_cl in  array_bd_cl.items():
    array_bd_cl[i] = sorted(lst_cl, key=lambda k: k['score'])  # ojo depende del caso
...
"""
   Almacenamos tod en grupos de clusters segun el ligando y el programa

             Cluster 1le0            58827677                 GLA              
                        0           AD_cl_1_9           AD_cl_1_8           
                              (1.2) LF_cl_1_4
...
for key, value in dict_clusters.items():
    header += format_text.format(key)    
header = format_text_coord.format("Coords")+header
print (header)
for key, value in dict_clusters.items():
    if len(value)> 1 :
        print (value[0])        
...
"""
    agrupamos centros de los cluterts y genera una hash con lo que se dibujara al final y se generara un json
"""
...
def get_format_cluster(cl, i ):
    ...
    #return format_text.format('{}_{} {} '.format(cl['lig_name'] ,cl['cluster'], str(cl['coords'])))
def get_format_cluster_latex(cl):
    return format_text.format('{}_{}_{}_{}'.format(cl['sw'], cl['cluster'], str(cl['score']), cl['lig_name']  ))
    #return format_text.format('{}_{}_{}'.format(cl['sw'], cl['cluster'], str(cl['score'])))
...
def get_groups(group, offset_right):
    ...
    for query, lst_group_clusters in dict_clusters.items():                     # iteramos en el diccionario de las columnas ordenadas por grupos

        if query not in lst_queries_off:	    								# para que no se repita query
...
def print_latex(text):
    """
        Es un poco lento abrir y cerrar fichero cada linea pero no quiero modificar mucho el codigo
    :param text: 
    :return: 
    """
    f = open(f_tex_out, 'a')
        ...
        if len (lst_group_clusters):
            for j in lst_group_clusters:
                if lst_group_clusters[j] not in lst_groups_off:
                    ...
                    #
                    #   Quiere decir que existe un solapamiento con un ligando y dos clusters
                    #
...
#    json.dump(dict_clusters, indent=4, json_file,sort_keys=True)
