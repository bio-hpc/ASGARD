...
"""
    ...
    Description: Join experiments conducted with LS by shuttlemol in a single file to visualize them with ligandscout
...
TAG_END_MOL = '$$$$'
TAG_SCORE = '> <Score>'
TAGS_NAME = [ 'DATABASE_ID',  'VillaPharma_Code' ] #special case for the name of the molecules VP and drugbank
SEARCH_MATCHES = "ShuttleMol/extra_shuttlemol/scopus/scopus_search_matches.py"
PYTOHN_RUN = "python"
F_SEARCH_MATCHES = PYTOHN_RUN+" "+SEARCH_MATCHES+" \'{}\'" 
...
def wirte_excel(lst_molecules, prefix):
	out = "ls_lst_"+prefix+"_all"
	f = open(out+".csv", 'w')
...
def get_name_molecule(molecule):
    ...
    #if (str(molecule[0].strip()).startswith("0")):
    #	print (molecule)
    if molecule[0] != '':
        return molecule[0]
    else:
...
parser = argparse.ArgumentParser(
    ...
    epilog="""Example of use: 
           python ShuttleMol/extra_shuttlemol/results/join/join_ls_sessions.py LS_6ioz_LBVS_with_excl_v1_DBALLv503_a_""",
    formatter_class=partial(argparse.ArgumentDefaultsHelpFormatter, width=500)
)
parser.add_argument('prefix', type=str, help='Prefix of imput dirs')
parser.add_argument('-c', type=float, help='cut-off', default=0.7)
parser.add_argument('-q', '--query', type=str, help='Query to search in scopus', default="")
...
wirte_excel(lst_excel, args.prefix)
