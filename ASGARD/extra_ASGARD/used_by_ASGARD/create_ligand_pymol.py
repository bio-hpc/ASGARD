...
# -*- coding: utf-8 -*-
#
#   Genera las sentencias pml necesarias para ver un ligando con susu interaciones en pymol
#
import sys
import json
import os
...
add_paths()
#
#   ojo para ver el cubo el pml tiene que tener
#   from pymol.cgo import *    # get constants
#


# color y foram de interacciones que se pintaran en el pml
F_CONFIGHOLDER = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))+"/analyze_results/Get_histogram/ConfigHolder.py" #sirve para saber que energia se esta usando
...
def get_cube_pymol(json_simulation):
    """
        Genera un cubo para pymol con el centro donde empezo el docking el ligando
    """
...
def draw_energy(energy, n_lines):
    """
        Dibujha una linea entre 2 puntos
...
def paint_scores(table_scores, json_simulation ):
    """
            Dibuja los scores de plip
    """
    #print (json_simulation['file_ori_target'])
    ...
    for name_energy, energies in table_scores['interactions_groups'].items():
        ...
        #lst.append("cmd.show('sticks', '" + n_sticks + "')")
...
#
# Si existe se coge primero el de quantica sino el de dmd y si no existe ninguno el de docking
#
...
#pml.append("cmd.group('{0}','Gr{4} {2} {1} {3}'  )".format(cl_group, json_simulation['file_name'], cl_name,
#str_interactions, num_site))
