...
function read_file()
{
	...
	path_shuttlemol=`cat ${file_conf} |grep "path_shuttlemol" |cut -d\  -f 2`
	...
	path_extra_shuttlemol=`cat ${file_conf} |grep "extra_shuttlemol" |cut -d\  -f 2`
    ...
    modules_shuttlemol=`sed -n  '/Shuttlemol_mol/,/Shuttlemol_get_histogram/p' ShuttleMol/config.cfg |grep -v \# | paste -sd\;`
    modules_get_histogram=`sed -n  '/Shuttlemol_get_histogram/,/end_modules/p' ShuttleMol/config.cfg |grep -v \# | paste -sd\;`
    #sed ':a;N;s/\n/; /g'`
