...
def get_interactions_residues(lst_in, mode):
    """
...
def create_energies_xvg(edr_rerun, tpr_min, g_energy, folder_hbonds, mode):
    ...
    lst = get_interactions_residues(output.decode("utf-8").split("\n"), mode)
    for i in range(0, len(lst), 4):
        ...
        #print(aux)
        cmd = 'echo "{} 0" | {} -f {} -o {}'.format(SPACES.join(aux), g_energy, edr_rerun, os.path.join(folder_hbonds, title_no_group))
