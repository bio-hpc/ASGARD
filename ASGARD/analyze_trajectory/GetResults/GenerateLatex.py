...
class GenerateLatex(object):
    ...
    def put_figure_2 (self, document, figure_a, figure_b, capt_a, capt_b, format_x):
        document.write(format_x.format('\\begin{figure}[H]'))
        document.write(format_x.format('\\begin{minipage}{0.48\\textwidth}'))
        document.write(format_x.format('\centering'))
    ...
    def rmsd(self, document):
        if self.cfg.p_graph_rmsd:
            ...
            for mol in self.cfg.lst_molecules:
                ...
                img_b = self.cfg.f_molecule_rmsd_distribution_png.format(self.cfg.prefix_results_png, mol.original_name)
            ...
            if self.cfg.p_graph_rmsdf:
                ...
                for i in range(len(self.cfg.lst_molecules)):
                    if i+1 < len(self.cfg.lst_molecules) and not i in lst_figures_put:
                        ...
                        img_a = self.cfg.f_molecule_rmsd_f_png.format(self.cfg.prefix_results_png, self.cfg.lst_molecules[i].original_name )
                        img_b = self.cfg.f_molecule_rmsd_f_png.format(self.cfg.prefix_results_png, self.cfg.lst_molecules[i+1].original_name)
                        self.put_figure_2(document, img_a, img_b, "", "", f_4)
            ...
            if self.cfg.p_desglose:
                ...
                if len(self.cfg.lst_molecules) > 2:  # si solo son 2 con las graficas anterirores vales
                    for i in range(1, len(self.cfg.lst_molecules)):
                        ...
                        img_b = self.cfg.f_molecule_distance_distribution_png.format(self.cfg.prefix_results_png, mol_target.original_name, mol_query.original_name)
                        self.put_figure_2(document, img_a, img_b,  "", "", f_4)
    ...
    def dssp(self,document):
        if self.cfg.p_graph_dssp:
            self.put_figure(document, self.cfg.out_png_ddsp.format(self.cfg.prefix_results_png), f_4, "Number of amino acid residues in each secondary structure type, as defined by DSSP, along the simulation time.")
            self.put_figure(document, self.cfg.out_png_ddsp_ss, f_4, 'Evolution of secondary structure, calculated by DSSP, as a function of both simulation time and amino acid residue position on the polypeptidic chain'  )
    ...
    def all_interactions(self, document):
        ...
        for i in range(1, len(self.cfg.lst_molecules)):
            ...
            if self.cfg.p_graph_hbonds or self.cfg.p_graph_mmpbsa:
                img_a = self.cfg.format_file_name_g_mmpbsa_png.format(self.cfg.prefix_results_png, mol_target.original_name,
                                                                  mol_query.original_name)
                img_b = self.cfg.format_file_name_hbonds_png.format(self.cfg.prefix_results_png,mol_target.original_name, mol_query.original_name)
    ...
    def generate_setup(self, document):
        ...
        document.write(f_2.format('\\footnotesize {'))

        #document.write(f_3.format('\\hspace{-0.5cm}'))
        document.write(f_4.format('\\begin{tabular}{ l r | l r | l r }'))
    ...
    def generate_head(self, document):
        ...
        document.write('\usepackage{geometry} \geometry { a4paper, total = {170mm, 235mm}, left = 20mm, top = 10mm, } \n')

        document.write('\usepackage{fontspec} \n')
        document.write('\usepackage{polyglossia} \n')
        document.write('%\setdefaultlanguage{english} \n')
        document.write('\usepackage{amsfonts} % for the \checkmark command  \n')
        document.write('\usepackage{textpos} \n')
        document.write('\usepackage{sectsty} \n')
        document.write('\usepackage{multicol} \n')
        document.write('\usepackage{enumitem} \n')
        document.write('\usepackage{graphicx} \n')
        ...
        document.write('\def\columnseprulecolor{\color{black}} \n' )
        document.write('\\lhead{\includegraphics[width = 4cm]{'+self.cfg.logo_bio_hpc+'}} \n')
        document.write('\\title{Simulation Report '+footer+'} \n')
        document.write('\\author{bio-hpc} \n')
        document.write('\\titlepic{ \\vspace{8cm} \includegraphics[width =\\textwidth]{'+self.cfg.md_diagram+'}}\n')
