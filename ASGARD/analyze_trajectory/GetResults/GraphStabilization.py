...
class GraphStabilization:
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.p_graph_stabilization:
            ...
            with open(self.cfg.table_stabilization, "w") as f:
        ...
        with open(file) as f:
            for i in f:
                if i.startswith("@    yaxis  label "):
                    i = re.sub(' +', ' ', i).strip()
                    aux = i.split(" ")
                    label = aux[3].strip()[2:-2]
                    label = label.replace("^", "\\^")
                if not i.startswith("#") and not i.startswith("@"):
                    i = re.sub(' +', ' ', i).strip()
                    aux = i.split(" ")
                    lst.append(float(aux[1]))
        return lst, label
